...
(async function () {
  ...
  try {
    ...
    const msgs = await window.api.messages.listLastMessagesForThreads(threadIds);
    ...
    const profiles = await window.api.profiles.getMany(otherIds);
    ...
    for (const t of threads) {
      ...
      const displayName = other?.full_name || other?.username || 'Conversation';
